...
環境変数 \texttt{XCRJOBSCHED} を適切に設定する．シェルが bash なら，
\begin{screen}
...
NQS を使う場合は
\begin{screen}
\texttt{\$ XCRJOBSCHED=NQS; export XCRJOBSCHED}
\end{screen}
とする．
...
\section{\texttt{stdodlmtr}}

実行プログラムの標準出力ファイルの行を区切る際の区切り文字を指定する．
デフォルト値は \texttt{,} である．

\section{\texttt{stdoclmn}}

実行プログラムの標準出力ファイルの列を指定する．デフォルト値は \texttt{0} である．
...
\begin{boxnote}
\begin{alltt}
prepare(ジョブ定義ハッシュ[,'RANGE0' => (リファレンス||スカラ)]
                      ...[,'RANGE255' => (リファレンス||スカラ)]
