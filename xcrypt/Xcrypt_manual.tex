...
例えば
\begin{center}
\texttt{\$separation\_symbol = '-';}
\end{center}
と記述する．セット可能な大域変数に関しては\ref{globalvar}で述べる．
...
\section{\texttt{\$opt\_dry}}

$1$ にセットすることで Xcrypt をドライモード（実行コマンドなしで）で動
作させる．

\section{\texttt{\$opt\_sge}}

ジョブスケジューラが Sun Grid Engine の場合は $1$ にセットすることで
Xcrypt を正常に動作させることができる．デフォルト値は$0$である．Xcrypt
実行時にオプション「\texttt{-s}」をつけることと同義である．
