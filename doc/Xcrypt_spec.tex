...
にワーキングディレクトリを移した後，\dollarit{command}をシェルコマンドとして
実行する．返り値は不定である．
%% note: 終了コードを返すのは，Net::OpenSSHのsystemメソッドの仕様により困難．
...
\item 文字列\|'cd |\mboxit{wkdir}\|'|に改行文字を加えた文字列をスクリ
  ...
  ある．
  \begin{itemize}
    \item $\mboxit{CFG}(\mboxtt{jobscript\_workdir})$の値が
      文字列の場合，その値．
    \item $\mboxit{CFG}(\mboxtt{jobscript\_workdir})$の値が
      関数の場合，その関数を\dollarit{self}を引数として呼び出した
      返り値の文字列．
    \item $\mboxit{CFG}(\mboxtt{jobscript\_workdir})$の値が未定
      義の場合，環境オブジェクト\dollarit{self}\|->{env}|に対応するワー
      キングディレクトリと\dollarit{self}\|->{workdir}|を連結したパス
      名．なお，$\mboxit{CFG}(\mboxtt{jobscript\_workdir})$の値が
...
\subsection{\texttt{xcrypt}}
\subsection{\texttt{xcryptstat}}
\subsection{\texttt{xcryptdel}}
