...
\begin{boxnote}
\begin{verbatim}
addexpandedkeys('userdefinedkey0', 'userdefinedkey1');
\end{verbatim}
\end{boxnote}
...
&create_submit_sync(%xyz, 'arg1@' => [2,4]);
\end{verbatim}
\end{boxnote}
...
@results = &create_submit_sync(%xyz, 'arg3@' => ['komadoku',
				                  'kuraidori',
				                  'ikiataribattari']);
\end{verbatim}
\end{boxnote}
...
\begin{boxnote}
\begin{verbatim}
create(ジョブ定義ハッシュ[,'RANGE0' => (配列リファレンス)]
                      ...[,'RANGE255' => (配列リファレンス)]
...
\begin{boxnote}
\begin{verbatim}
@jobs = create('id' => 'xyz', 'exe' => './kempo', 'arg0@' => [10,20]);
\end{verbatim}
\end{boxnote}
...
\begin{boxnote}
\begin{verbatim}
@jobs = create(%abc, 'RANGE0' => [0..99], 'arg0@' => sub { 2 * $_[0] });
\end{verbatim}
\end{boxnote}
は\texttt{create(\%abc, 0), create(\%abc, 2),..., create(\%abc,
198)}を順番に行ったものと同義である．
...
@jobs = create(%abc, 'arg0@' => [0,1], 'arg1@' => [2,3]);
\end{verbatim}
\end{boxnote}
は以下と同義である．
\begin{boxnote}
...
@jobs = create(%abc, 'RANGE0' => [0,1], 'RANGE1' => [2,4],
                'arg0@' => sub { $_[0] + $_[1] });
\end{verbatim}
\end{boxnote}
は以下と同義である．
...
\section{\texttt{addexpandedkeys}}

文字列の配列を受け取り，それらを\texttt{create}で展開されるジョブ定義ハッ
シュキーとして利用可能にする．
...
\begin{boxnote}
\begin{verbatim}
addexpandedkeys(文字列の配列);
\end{verbatim}
\end{boxnote}

\section{\texttt{create\_submit}}

\texttt{create}，\texttt{submit}を順に行う．ただし，\texttt{create}で
生成したジョブリファレンスを即座に\texttt{submit}する（全てのジョブリファ
レンスが生成されるのを待たない）．\format は\texttt{create}に準ずる．
...
\section{\texttt{create\_submit\_sync}}

\texttt{create\_submit}，\texttt{sync}を順に行う．\format は
\texttt{create}に準ずる．
