...
\texttt{t2k-tsukuba}, \texttt{t2k-tokyo}, \texttt{t2k-kyoto}, and
\texttt{sh} are available.  In the case of \texttt{sh}, jobs are dealt
with as processes in OS.  The default is \texttt{sh}} should be set to
your job scheduler, and
\begin{screen}
...
Optionally, set \texttt{XCRQUEUE} as the default queue.
...
unfinished) jobs aborted, use the \texttt{--cancel} option.  For
making jobs finished, use the \texttt{--invalidated} option.  For
