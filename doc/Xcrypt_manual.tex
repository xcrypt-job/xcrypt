...
Xcrypt requires Perl ($\geq$ 5.8.5) and a superset of Bourne shell.
...
\item H.Merijn Brand and Jochen Wiedmann's Text-CSV\_XS,
\end{itemize}
...
Xcrypt's job objects are implemented as Perl's objects (blessed hash
...
Xcrypt's environment objects are implemented as Perl's objects
(blessed hash references).  In Xcrypt, environment objects should be
typically created from hash references by a built-in function
\texttt{\&add\_host} (Chapter~\ref{chapfun} in detail).
...
See the \texttt{xcrypt/INSTALL} document.
...
%% If necessary, edit \texttt{\$XCRYPT/etc/xcryptrc} along your
%% environment.

%% Edit \texttt{\$XCRYPT/bin/.wrapper} along your environment for the
%% purpose of making it possible to skip setting user environment variables.

\section{Execution}

If necessary, edit \texttt{\$HOME/.xcryptrc} along your environment.

\if0
Edit \texttt{\$HOME/.xcryptrc} as follows,
\begin{boxnote}
\begin{verbatim}
[default]
sched = t2k_kyoto
...
\end{boxnote}
\noindent
along your environment\footnote{\texttt{SGE}, \texttt{t2k\_tsukuba},
\texttt{t2k\_tokyo}, \texttt{t2k\_kyoto}, and \texttt{sh} are
available.  In the case of \texttt{sh}, jobs are dealt with as
processes in OS.  The default is \texttt{sh}}.
\fi
...
For advanced users, a more sophisticated module \texttt{bulk.pm} is
also provided.  In detail, see \texttt{doc/bulk\_module\_manual.pdf}.  \fi

\section{\texttt{bulk}}

This module gathers jobs and returns one job of the same denotation.
In detail, see the manual \texttt{bulk\_module\_manual.pdf}.
...
Its value is an array.  The value is used for creating job objects.
In detail, see Section~\ref{sec:prepare}.
...
\if0
\section{\texttt{RANGES}}

Its value is an array of arrays (of values of \texttt{RANGE}\textit{i}s).
\fi
...
\begin{screen}
\texttt{\$ perl} \textit{exe.pl}
\end{screen}
where \textit{exe.pl} consists of only the function.
...
Its value denotes environments by which jobs generated by templates
are surrounded.  By default, an environment (denoting a local-modeemb
Xcrypt execution) is embedded into templates.  When you intend to
execute jobs at remote hosts, it is enough to embed such an
environment (typically returned by the embedded function
\texttt{add\_host}).  In detail, see \texttt{add\_host} in
Section~\ref{chapfun}.
...
\vspace{\baselineskip}
\noindent
where \texttt{\$VALUE[0]} refers to an element of the value (i.e.,
array) of \texttt{RANGE0}.
...
\vspace{\baselineskip}
\noindent
creates 4 job objects $(0,2), (0,4),(1,2), (1,4)$ which can
be referred by \texttt{\$VALUE[0]} and \texttt{\$VALUE[1]},
respectively.
...
Entries of \texttt{host} and \texttt{sched} are required.  By
default, \texttt{wd} and \texttt{xd} have the values of \texttt{\$HOME} and
\texttt{\$XCRYPT} as values, respectively.
...
\begin{boxnote}
\begin{alltt}
$env = add_host(\{'host' => '\texttt{foo@bar}',
                 'sched' => '\texttt{torque}',
                 'wd' => '\texttt{/home/foo/myapp}',
                 'xd' => '\texttt{/usr/local/share/xcrypt}'\});
...
\begin{boxnote}
\begin{alltt}
$env = add_host(\{'host' => '\texttt{foo@bar}', 'sched' => '\texttt{torque}'\});
%template = ('id' => '\textit{myjob}', 'exe0' => '\textit{./myexe}', 'env' => $env);
\end{alltt}
\end{boxnote}
...
\section{\texttt{--config} \textit{xcryptrc}}

reads the \textit{xcryptrc}, \texttt{\~{}/.xcryptrc}, or \texttt{\$XCRYPT/etc/xcryptrc}.

\section{\texttt{--sched} \textit{job\_scheduler}}
...
\section{\texttt{--xbs-type} \textit{type}}

use Keiji Yamamoto's
XBS\footnote{\texttt{http://www.il.is.s.u-tokyo.ac.jp/\~{}yamamoto/xbs/}}
internally for generating job scripts of the
\textit{type}.

\section{\texttt{--xbs-auto}}

use XBS internally for generating job
scripts (of an inferred type).


\section{\texttt{--abort\_check\_interval} \textit{num}}

an interval of checking whether jobs are aborted or not (sec).

\section{\texttt{--left\_message\_check\_interval} \textit{num}}

an interval of checking what states jobs have (sec).

\section{\texttt{--inventory\_path} \textit{path}}

a path at that logs are located.

\section{\texttt{--verbose} \textit{num}}

\section{\texttt{--stack\_size} \textit{num}}

\section{\texttt{--host} \textit{username@hostname}}

a host to that jobs are submitted.

\section{\texttt{--wd} \textit{path}}

a working directory in that Xcrypt is executed at a remote host.

\section{\texttt{--xd} \textit{path}}
...
\if0
\subsection{\texttt{before\_in\_job}}

The definition of this method is serialized (using the Data::Dumper
module), sent to a job execution node, and interpreted and invoked by
the Perl process executed in the node just {\em before} executing
\texttt{exe}\textit{i}.  The job object is also serialized and passed
to the method as its argument. A global variable defined in (local)
Xcrypt is referred to only if it is specified by the
\texttt{transfer\_variable} job template member.  Updates of the
transferred variable are not reflected to the Xcrypt process.

\subsection{\texttt{after\_in\_job}}

The definition of this method is serialized (using the Data::Dumper
module), sent to a job execution node, and interpreted and invoked by
the Perl process executed in the node just {\em after} executing
\texttt{exe}\textit{i}.  The job object is also serialized and passed
to the method as its argument. A global variable defined in (local)
Xcrypt is referred to only if it is specified by the
\texttt{transfer\_variable} job template member.  Updates of the
