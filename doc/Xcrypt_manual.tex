...
Xcrypt requires
\begin{itemize}
\item Marc Lehmann's Coro (Perl Artistic License, Coro/libcoro under GPL),
      EV (Perl Artistic License, libev under the 2-clause BSD),
\item Joshua Nathaniel Pritikin's Event (?), and
\item Daniel Muey's Recursive (Perl Artistic License),
\end{itemize}
...
procedure:
\begin{itemize}
\item to create a script in its writing style depending on what the
      job scheduler is,
\item to pass the script to the job scheduler, and
\item to extract data from its result, to create another script from
      the data, and to pass it to the job scheduler.
\end{itemize}

However, such a procedure requires manual intervention cost.  It
...
Xcrypt is a script language, an extension of Perl.  Xcrypt provides
some functions and modules (not in Perl) which supports how to deal
with \textit{jobs}.
...
\vspace{\baselineskip}
\noindent
Every module is used in order.  In Chapter~\ref{chapmodule}, modules
...
Some modules define variables.  In order to use such variables, it is
enough to write e.g.,
\begin{boxnote}
\begin{verbatim}
$separator = '-';
...
Procedure (usually manually done) are written in Xcrypt (and Perl).
...
\texttt{XCRYPT} is set to the directory (e.g.,
\texttt{/usr/share/xcrypt}) in which Xcrypt is installed.  For
example, it is enough to type
\begin{screen}
...
is also available (by being set to \texttt{sh}) for environments
without any job scheduler.  In that case, jobs are dealt with as
processes in OS.} is set to a job scheduler.  \texttt{PERL5LIB} is set
to \texttt{\$XCRYPT/lib} and \texttt{\$XCRYPT/lib/algo/lib}.
...
is created each job (called a \textit{job working directory}).  It is
...
This module provides how to write dependency of jobs declaratively.
...
Various job objects can be generated by using
...
This function takes an array of job objects, passes the jobs
(corresponding to the job objects) to a job scheduler.  Its return
value is no change (the array of job objects).
...
This function takes an array of job objects, syncs the job objects.
Its return value is no change (the array of job objects).
...
This function takes a Xcrypt's script code (written as
\textit{mystring}) and an integer $i$, and evaluates it each the $i$
seconds.
