# Config file for the Subsystem G (Gardenia, DELL PowerEdge XE8545)
...
my $NCORE = 64;  # cores per physical node
my $NGPU  =  4;  # GPUs per physical node
...
$jsconfig::jobsched_config{$myname} = {
    ...
    jobscript_other_options => sub {
        ...
        my $gpu = $self->{JS_gpu} || 0;
        my $thread = $self->{JS_thread} || $cpu;
        my $memory = $self->{JS_memory} || floor($MEM*$cpu/$NCORE).'M';
        if ($cpu < ($NCORE/$NGPU)*$gpu) {
            $cpu = ($NCORE/$NGPU)*$gpu;
        }
        # On Gardenia, gpus are allocated according to the number of cpus.
