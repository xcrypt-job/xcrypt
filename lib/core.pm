...
sub new {
    ...
    set_member_if_empty ($self, 'injob_program', 'perl');
...
#    push (@body, "sleep 1"); # running ���᤹���� queued ���ʤ��ʤ����Ƥʤ�����
    ...
    push (@body, "$self->{injob_program} $self->{before_in_job_file}");
    # Execute the program
    ...
    if ( defined $self->{exe} ) {
        ...
        if ( ref $self->{exe} eq 'CODE' ) {
            ...
            push (@body, "$self->{injob_program} $self->{exe_in_job_file}"); 
        } else {
...
sub make_before_in_job_script {
    ...
    $self->make_in_job_script ('before_in_job_script', @names);
}
...
sub make_after_in_job_script {
    ...
    # Calls it even if @names is empty because child methods may add code
    $self->make_in_job_script ('after_in_job_script', @names);
}
